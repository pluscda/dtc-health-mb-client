<template>
  <div class="my-homepage">
    ...
    <main class="hot-search">
      <div></div>
      <div>{{ $t("熱搜") }}:</div>
      <div v-for="(item, i) in cancers.slice(5, 10)" :key="i" style="margin-left:7px;color:blue;font-size:14px;" @click="$router.push(`doclist?searchBy=${item.name}`)">
        {{ $t(item.name) }}
      </div>
    </main>
...
<i18n>
{
  "zh-tw": {
    ...
    "關愛女性健康":"關愛女性健康",
    "愛情急診室":"愛情急診室",
    "省時省心名醫指導好的快":"省時省心 名醫指導好的快",
    "月經異常":"月經異常",
    "伴侶出軌":"伴侶出軌",
    "自選科室專屬醫師對症下藥":"自選科室 專屬醫師 對症下藥"
  },
  "zh-cn": {
    "快速問醫生": "快速问医生",
    "oneMin":"1分钟速配医生",
...
const onlines = ["家醫科", "老年醫學科", "神經科", "泌尿科", "眼科", "婦產科", "皮膚科", "小兒科"].map((s, i) => ({
  src: s,
  text: strs[i],
}));
export default {
